...
class Signer(_PrehashingAuthenticator):
    ...
    def sign(self, data):
        hasher = self._build_hasher()
        hasher.update(data)
        return _ecc_static_length_signature(key=self.key, algorithm=self.algorithm, digest=hasher.finalize())
...
class Verifier(_PrehashingAuthenticator):
    ...
    def verify(self, data, signature):
        ...
        hasher = self._build_hasher()
        hasher.update(data)
        prehashed_digest = hasher.finalize()
        self.key.verify(
            signature=signature,
...
def load_private_key(keypath):
    with open(os.path.expanduser(keypath)) as f:
        key_bytes = ''.join(f).strip().encode()
        return serialization.load_pem_private_key(data=key_bytes, password=None, backend=default_backend())


def load_public_key(keypath):
    with open(os.path.expanduser(keypath)) as f:
        key_bytes = ''.join(f).strip().encode()
        return serialization.load_pem_public_key(data=key_bytes, backend=default_backend())
